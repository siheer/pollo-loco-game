...
export default class Coin extends GameItem {
     ...
     * @param {Level} level - The level instance in which the coin exists.
    ...
    constructor(level, segmentIndex, y, width, height) {
        const randomX = level.getRandomXInSegment(segmentIndex);
        const randomY = y -= Coin.getRandomY(level);
     ...
     */
    static getRandomY(level) {
        return Math.random() * (level.canvas.height / 2.5);
    }
}